...
const FEEDBACK_DEFAULTS_FR: Record<FeedbackTag, string> = {
  ...
  'conclusion-does-not-follow': 'Cette étape ne permet pas d’obtenir la formule attendue.',
};
...
const EXERCISE_COPY: Record<Locale, Record<string, ExerciseCopy>> = {
  en: {
    ...
    'tt-001': { prompt: 'Choose the missing value.', cellCorrect: 'Correct — conjunction requires both conjuncts to be true.', cellWrong: 'Under this assignment, P is true and Q is false, so P ∧ Q is false.' },
  ...
  fr: {
    ...
    'eval-013': {
      prompt:
        'Une disjonction n\'est fausse que si ses deux termes sont faux. Essayez P et Q faux et lisez le résultat.',
      assessmentPrompt:
        'Fixez P et Q, suivez les valeurs intermédiaires visibles, puis prédisez la valeur au connecteur principal.',
      hint: 'Une disjonction n\'est fausse que si ses deux termes sont faux.',
    },
    'eval-014': {
      prompt:
...
const UI: Record<Locale, UiCopy> = {
  en: {
    ...
    evaluationPracticePrompt: 'Given the fixed truth assignment below, follow the visible intermediate values, then predict the truth value at the root.',
  ...
  fr: {
    ...
    evaluationPracticePrompt: 'Étant donné l\'interprétation fixée ci-dessous, suivez les sous-formules visibles, puis prédisez la valeur de vérité au connecteur principal.',
...
export function getAssessmentPrompt(
  ...
  if (exerciseType === 'evaluate-formula' && assignment) {
    return formatEvaluationAssessmentPrompt(locale, assignment);
  }
...
const PROGRESS_UI: Record<Locale, ProgressUiCopy> = {
  en: {
    ...
    exercisesStatus: (done, unlocked) => `${done} completed · ${unlocked} available`,
    ...
    exerciseLockedUnit1: 'Complete Unit 1',
    exerciseLockedUnit2: 'Complete Unit 2',
    ...
    whatNextWeakestSkill: (skill, rate) => `Recent success rate for ${skill}: ${rate}% — a few targeted exercises should help.`,
  ...
  fr: {
    ...
    level1Heading: 'Unité sur les connecteurs',
    level1Status: (done, total) => `${done} section${done > 1 ? 's' : ''} sur ${total} terminée${done > 1 ? 's' : ''}`,
    exercisesHeading: 'Exercices',
    exercisesStatus: (done, unlocked) => `${done} terminé${done > 1 ? 's' : ''} · ${unlocked} disponible${unlocked > 1 ? 's' : ''}`,
    ...
    lessonTodo: 'à faire',
    exerciseDone: 'réussi',
    exerciseAttempted: 'tenté',
    exerciseLocked: 'verrouillé',
    exerciseLockedUnit1: 'Terminer l’unité 1',
    exerciseLockedUnit2: 'Terminer l’unité 2',
    ...
    whatNextResumePractice: 'Reprendre votre dernière séance d’exercices.',
...
const VISIBILITY_UI: Record<Locale, VisibilityUiCopy> = {
  en: {
    ...
    sessionProgressAria: (completed, target) =>
      `Practice session: ${completed} of ${target} exercises completed`,
    ...
    youCanNowHeading: 'Consistent so far',
    youCanNowEmpty:
    ...
    momentExerciseUnlocked: (exercise) => `Next exercise available: ${exercise}.`,
  ...
  fr: {
    ...
    stateConsistent: 'Résultats réguliers',
    capabilityStatusAria: (name, state) => `${name} : ${state}`,
    sessionProgress: (completed, target) => `${completed} / ${target}`,
    sessionProgressAria: (completed, target) =>
      `Séance d’exercices : ${completed} exercice${completed > 1 ? 's' : ''} terminé${completed > 1 ? 's' : ''} sur ${target}`,
    sessionCompleteHeading: 'Séance terminée',
    sessionCompleteCount: (count) =>
      count === 1
        ? 'Vous avez terminé 1 exercice pendant cette séance.'
        : `Vous avez terminé ${count} exercices pendant cette séance.`,
    sessionPractised: (names) => `Notions travaillées : ${names}.`,
    ...
    momentExerciseUnlocked: (exercise) => `Nouvel exercice disponible : ${exercise}.`,
    momentScaffoldAdvanced:
