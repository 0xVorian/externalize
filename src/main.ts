...
root.addEventListener('click', (event) => {
  ...
  if (action === 'palette-insert') { practiceState = paletteInsertToken(ensurePracticeState(), button.dataset.token, button.dataset.value); render(); return; }
  if (action === 'palette-backspace') { practiceState = paletteBackspace(ensurePracticeState()); render(); return; }
  if (action === 'palette-undo') { practiceState = paletteUndo(ensurePracticeState()); render(); return; }
  if (action === 'check-translation') { practiceState = checkTranslation(ensurePracticeState()); if (practiceState.phase === 'answered' && practiceState.feedback) { persistProgress(recordResult(progress, practiceState.exercise.id, practiceState.feedback.correct, practiceState.feedback.correct ? undefined : practiceState.feedback.tag)); refreshPracticeQueue(); } render(); return; }
  if (action === 'next') {
    advancePractice();
  }
});
