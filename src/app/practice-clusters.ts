...
export function unit1ClusterForExercise(
  exerciseId: string,
): keyof typeof UNIT_1_PRACTICE_CLUSTERS | undefined {
  for (const key of UNIT_1_CLUSTER_ORDER) {
    if ((UNIT_1_PRACTICE_CLUSTERS[key] as readonly string[]).includes(exerciseId)) {
      return key;
    }
  }
  return undefined;
...
export function unit1ClusterOrder(exerciseId: string): readonly string[] | undefined {
  const key = unit1ClusterForExercise(exerciseId);
  return key ? UNIT_1_PRACTICE_CLUSTERS[key] : undefined;
}
