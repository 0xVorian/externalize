...
describe('computeWhatNext',()=>{
  ...
  it('next exercise',()=>{let s=cl(loadProgress());s=attempt(s,true);const next=computeWhatNext('en',s,buildSummaryFromStore(s));expect(next.exerciseId).toBe('scope-012');expect(next.detail).not.toContain('scope-012');expect(next.detail).toContain('Main connective');});
});
