...
function renderTreeNode(node: TreeNode, state: AppState): string {
  ...
  if (state.exercise.type === 'identify-main-connective') classes.push('tappable');
  ...
  return `<li class="${classes.join(' ')}" data-node-id="${node.id}"><button type="button" class="node-button" data-action="select-node" data-node-id="${node.id}"><span class="node-label">${node.label}</span>${valueHtml}</button>${children ? `<ul class="tree-children">${children}</ul>` : ''}</li>`;
}
...
function renderEvaluationBody(state: AppState): string {
  if (state.exercise.type !== 'evaluate-formula') return '';
  ...
  return `<section class="tree-panel" aria-label="${ui(state.locale).formulaTreeAria}"><ul class="tree-root">${renderTreeNode(state.tree, state)}</ul></section>${renderAtomToggles(state)}`;
}
...
function renderExerciseBody(state: AppState): string {
  if (state.exercise.type === 'translate-en-to-formula') return renderTranslationExerciseBody(state);
...
export function renderApp(state: AppState, queueSize: number, practiceUnlocked: boolean): string {
  ...
  const formulaLine = state.exercise.type === 'translate-en-to-formula' ? '' : `<p class="formula-display" aria-label="Formula">${state.exercise.formula}</p>`;
  return `<main class="app" lang="${state.locale}">${renderShellHeader({ locale: state.locale, mode: 'practice', practiceUnlocked, title: copy.practice, meta: copy.queueMeta(queueSize), referenceOpen: false })}<article class="exercise-card"><p class="exercise-prompt">${state.prompt}</p>${formulaLine}${renderExerciseBody(state)}${state.message ? `<p class="feedback ${feedbackClass}" role="status">${state.message}</p>` : ''}<div class="actions">${state.exercise.type === 'identify-main-connective' && state.phase === 'answered' ? `<button type="button" class="primary" data-action="next">${copy.continue}</button>` : ''}${state.exercise.type === 'evaluate-formula' ? `<button type="button" class="secondary" data-action="next">${copy.nextExercise}</button>` : ''}${state.exercise.type === 'translate-en-to-formula' ? renderTranslationActions(state) : ''}</div></article></main>`;
}
